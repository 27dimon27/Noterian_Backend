...
const (
	...
	CREATE_NOTE = `
		INSERT INTO notes (user_id, title, parent_id, created_at, updated_at) 
		VALUES ($1, $2, $3, now(), now()) 
		RETURNING id, user_id, title, parent_id, created_at, updated_at
	`

	UPDATE_NOTE = `
		UPDATE notes SET title = $2, parent_id = $3, updated_at = now() 
		WHERE id = $1 
		RETURNING id, user_id, title, parent_id, created_at, updated_at
	`
