...
func TestHub_HandleBroadcast_BlockedChannelTriggersFallback(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	env := newHubTestEnv(ctrl)
	...
	case msg := <-c.Send:
		if msg.Type != MsgHeartbeat {
			t.Errorf("expected heartbeat via fallback goroutine, got %s", msg.Type)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("fallback goroutine did not deliver message")
	}
}
