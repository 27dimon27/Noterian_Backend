...
CREATE TRIGGER update_block_formatting_updated_at 
    ...
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();CREATE EXTENSION IF NOT EXISTS pgcrypto;
...
SELECT setval('block_types_id_seq', (SELECT MAX(id) FROM block_types));CREATE USER noterian_app_user WITH PASSWORD 'noterian_app_password';
