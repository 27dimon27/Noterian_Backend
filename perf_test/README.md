...
Для всех из них требуется авторизация через JWT, а для путя с записью еще и CSRF-токен. Эту проблему, а также проблему получения списка ID заметок для теста чтения, решают вспомогательные скрипты.

Мы создаем 100 аккаунтов и затем в режиме round-robin создаем через них 100,000 заметок.
Потом мы читаем каждую заметку отдельно и наконец получаем список заметок для каждого пользователя.
...
Здесь и далее предполагается, что сервер запущен на том же компьютере, что проводит тест.

```sh
# Для начала, создаем аккаунты
# Мы забираем их JWT и CSRF, и записываем это все
# в NDJSON файл для vegeta с `POST /notes` запросами
go run ./prepare \
...
Результаты теста представленны в папке `results/first`
...
```
исправили данную проблему, дав нам следующие результаты (`results/second`):
```
...
на это приходится половина, если не более, ошибок в логе :-)
...
Однако даже те запросы, которые выполнились, были медленны.
Интереснее то, что запросы на, "более тяжелый" endpoint
...
Вероятной причиной этого является следующая часть функции `GetNote` в `internal/notes/usecase/notes.go`:
```go
